...
set(tac_error_checks  error_on_negative_array_size error_on_pointer_subtraction
...
function(check_cmpl status res out)
    if(NOT "${res}")
        ...
        if("${lrep}" MATCHES " error:")
            set(sts "ERRORS")
        elseif("${lrep}" MATCHES " warning:")
            set(sts "WARNINGS")
        else()
            set(sts "OK")
        endif()
...
foreach(chk IN ITEMS ${tac_checks} ${tac_error_checks})
    if("${chk}" MATCHES "^error_")
        try_run(run cmpl SOURCES "${TAC_SOURCE_DIR}/${chk}.c"
                COMPILE_DEFINITIONS "-DTAC_DONT_FAIL" ${TAC_DEFINITIONS}
                COMPILE_OUTPUT_VARIABLE cmpl_out
                RUN_OUTPUT_VARIABLE run_out
                )
        check_cmpl(sts "${cmpl}" "${cmpl_out}")
        if(NOT "${sts}" STREQUAL OK)
            message(FATAL_ERROR "${chk}: Broken compile, ${sts}, output:\n"
                                "${cmpl_out}")
        endif()
        if(NOT "${run}" EQUAL 0)
            message(FATAL_ERROR "${chk}: Broken pre-check, ${run}, output:\n"
                                "${run_out}")
        endif()
    endif()
    try_run(run cmpl SOURCES "${TAC_SOURCE_DIR}/${chk}.c"
            COMPILE_DEFINITIONS ${TAC_DEFINITIONS}
            COMPILE_OUTPUT_VARIABLE cmpl_out
            RUN_OUTPUT_VARIABLE run_out
            )
